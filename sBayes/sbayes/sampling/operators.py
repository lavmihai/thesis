...
class AlterCluster(ClusterOperator):
    ...
    def grow_cluster(
        ...
        assert np.all(cluster_posterior_back > 0) 
        assert np.all(cluster_posterior_back < 1)
    ...
    def shrink_cluster(
        ...
        log_q = np.log(p_remove[removed_objects] + 1e-6).sum() + log_q_s
        log_q_back = np.log(p_add[removed_objects] + 1e-6).sum() + log_q_back_s
